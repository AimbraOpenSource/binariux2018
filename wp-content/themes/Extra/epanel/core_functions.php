...
if ( ! function_exists( 'et_epanel_admin_js' ) ) {
	...
	function et_epanel_admin_js(){
		...
		wp_localize_script( 'epanel_functions_init', 'ePanelSettings', array(
			...
			'help_label'   => esc_html__( 'Ajuda', $themename ),
		));
	}
...
function et_add_epanel() {
	...
	$core_page = add_theme_page( $themename . ' ' . esc_html__( 'Opções', $themename ), $themename . ' ' . esc_html__( 'Opções do tema', $themename ), 'switch_themes', basename( __FILE__ ), 'et_build_epanel' );
...
if ( ! function_exists( 'et_build_epanel' ) ) {
	...
	function et_build_epanel() {
		...
		if ( isset($_GET['saved']) ) {
			if ( $_GET['saved'] ) echo '<div id="message" class="updated fade"><p><strong>' . esc_html( $themename ) . ' ' . esc_html__( 'configurações salvas.', $themename ) . '</strong></p></div>';
		}
		if ( isset($_GET['reset']) ) {
			if ( $_GET['reset'] ) echo '<div id="message" class="updated fade"><p><strong>' . esc_html( $themename ) . ' ' . esc_html__( 'Resetar configurações.', $themename ) . '</strong></p></div>';
		}
	?>
		...
		<div id="wrapper">
		  <div id="panel-wrap">
			...
			<div id="epanel-top">
				<button class="save-button" id="epanel-save-top"><?php esc_html_e( 'Salvar alterações', $themename ); ?></button>
			</div>
			...
			<form method="post" id="main_options_form" enctype="multipart/form-data">
						...
						<div id="epanel-content-wrap">
							<div id="epanel-content">
								<div id="epanel-header">
									<h1 id="epanel-title"><?php printf( esc_html__( '%s Opções do tema', $themename ), $themename ); ?></h1>
									...
									<a href="#" class="defaults-button epanel-reset" title="<?php esc_attr_e( 'Resetar para os padrões', $themename ); ?>"><span class="label"><?php esc_html_e( 'Resetar para os padrões', $themename ); ?></span></a>
								...
								<ul id="epanel-mainmenu">
									<?php if ( in_array( 'general', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-general"><?php esc_html_e( 'Geral', $themename ); ?></a></li>
									<?php } ?>
									<?php if ( in_array( 'navigation', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-navigation"><?php esc_html_e( 'Navegação', $themename ); ?></a></li>
									<?php } ?>
									...
									<?php if ( in_array( 'ad', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-advertisements"><?php esc_html_e( 'Publicidade', $themename ); ?></a></li>
									<?php } ?>
									<?php if ( in_array( 'colorization', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-colorization"><?php esc_html_e( 'Colorização', $themename ); ?></a></li>
									<?php } ?>
									...
									<?php if ( in_array( 'integration', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-integration"><?php esc_html_e( 'Integração', $themename ); ?></a></li>
									<?php } ?>
									<?php if ( in_array( 'support', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-support"><?php esc_html_e( 'Suporte', $themename ); ?></a></li>
									<?php } ?>
									<?php if ( in_array( 'updates', $epanelMainTabs ) ) { ?>
										<li><a href="#wrap-updates"><?php esc_html_e( 'Atualizações', $themename ); ?></a></li>
									<?php } ?>
				...
				<div id="epanel-bottom">
					...
					<button class="save-button" name="save" id="epanel-save"><?php esc_html_e( 'Salvar configurações', $themename ); ?></button>
		...
		<div class="et_pb_yes_no_button_wrapper">
			<div class="et_pb_yes_no_button"><!-- .et_pb_on_state || .et_pb_off_state -->
				<span class="et_pb_value_text et_pb_on_value"><?php esc_html_e( 'Ativado', $themename ); ?></span>
				<span class="et_pb_button_slider"></span>
				<span class="et_pb_value_text et_pb_off_value"><?php esc_html_e( 'Desativado', $themename ); ?></span>
			</div>
		</div>
		</script>
		...
		<style type="text/css">
			#epanel p.postinfo-author .mark:after {
				content: '<?php esc_html_e( "Autor", $themename ); ?>';
			}

			#epanel p.postinfo-date .mark:after {
				content: '<?php esc_html_e( "Data", $themename ); ?>';
			}

			#epanel p.postinfo-categories .mark:after {
				content: '<?php esc_html_e( "Categorias", $themename ); ?>';
			}

			#epanel p.postinfo-comments .mark:after {
				content: '<?php esc_html_e( "Comentários", $themename ); ?>';
			}
		</style>
...
function et_epanel_media_upload_scripts() {
	...
	wp_localize_script( 'et_epanel_uploader', 'epanel_uploader', array(
		'media_window_title' => esc_html__( 'Escolha uma imagem', $themename ),
	) );
}
...
function et_epanel_register_portability() {
	...
	et_core_portability_register( 'epanel', array(
		'name'    => sprintf(
			esc_html__( '%s Opções do tema', $themename ),
			$themename
		),
		'type'    => 'options',
