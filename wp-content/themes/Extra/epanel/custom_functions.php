	...
	function elegant_titles_filter( $custom_title ) {
		...
		if ( is_category() || is_archive() || is_search() || is_404() ) {
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				$page_title = sprintf( esc_html__( 'Resultados de busca para "%s"', $themename ), esc_attr( get_search_query() ) );
			} else if ( is_404() ) {
...
if ( ! function_exists( 'et_resize_image' ) ) {
	...
	function et_resize_image( $thumb, $new_width, $new_height, $crop ){
		...
		if ( !$size ) return new WP_Error( 'invalid_image_path', esc_html__( 'Essa imagem não existe' ), $thumb );
