...
function et_register_post_format_taxonomy(){
	...
	register_taxonomy( ET_POST_FORMAT, 'post', array(
		...
		'hierarchical'      => false,
		'labels'            => array(
			'name'          => esc_html_x( 'Formato', $shortname ),
			'singular_name' => esc_html_x( 'Formato', $shortname ),
		),
		'query_var'         => true,
		'rewrite'           => false,
		'show_ui'           => false,
...
function et_set_post_format( $post, $format ) {
	...
	if ( empty( $post ) )
		return new WP_Error( 'invalid_post', esc_html__( 'Post inválido' ) );
...
function et_get_post_format_strings() {
	$strings = array(
		...
		'gallery'  => esc_html__( 'Galeria', 'extra' ),
		'link'     => esc_html__( 'Link', 'extra' ),
		'image'    => esc_html__( 'Imagem', 'extra' ),
...
function et_register_writing_admin_settings() {
	...
	add_settings_field( 'et_default_post_format', esc_html__( 'Padrão para formato de postagem', 'extra' ), 'et_default_post_format_render', 'writing', 'et_writing_settings' );
