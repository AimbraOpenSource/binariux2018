...
function et_add_extra_menu() {
	...
	if ( et_pb_is_allowed( 'theme_options' ) ) {
		...
		add_submenu_page( 'et_extra_options', esc_html__( 'Opções do tema', 'extra' ), esc_html__( 'Opções do tema', 'extra' ), 'manage_options', 'et_extra_options' );
	}
...
class Extra_Post_Review_Meta_Box extends ET_Meta_Box {
	...
	function display( $post ) {
		...
		?>
			<div class="breakdown group" data-breakdown_id="<?php echo esc_attr( $index ); ?>">
				...
				<div class="content">
					...
					<p class="field_wrap">
						<label for="_post_review_box_breakdowns[<?php echo esc_attr( $index ); ?>][title]"><?php esc_html_e( 'Título:', 'extra' ); ?></label>
					...
					<p class="field_wrap">
						<label for="_post_review_box_breakdowns[<?php echo esc_attr( $index ); ?>][rating]"><?php esc_html_e( 'Classificação (%):', 'extra' ); ?></label>
		...
		<button class="button button-small add_breakdown"><?php esc_html_e( 'Adicionar repartição', 'extra' ); ?></button>
		<?php
	}
	...
	function fields() {
		$this->fields = array(
			'_blog_feed_page_display_style'       => array(
				'title'       => esc_html__( 'Exibir estilo', 'extra' ),
				'type'        => 'select',
				...
				'description' => esc_html__( 'Exibir método', 'extra' ),
			),
			'_blog_feed_page_categories'          => array(
				'title'                   => esc_html__( 'Categorias', 'extra' ),
				'type'                    => 'custom',
				'description'             => esc_html__( 'Escolha de categorias.', 'extra' ),
				'renderer'                => array(
					$this,
			...
			'_blog_feed_page_posts_per_page'      => array(
				'title'       => esc_html__( 'Posts por páginas', 'extra' ),
				'type'        => 'input',
				'description' => esc_html__( 'O número de páginas a serem exibidas por páginas.', 'extra' ),
			),
			'_blog_feed_page_orderby'             => array(
				'title'       => esc_html__( 'Método de classificação', 'extra' ),
				'type'        => 'select',
				'default'     => 'date',
				'options'     => array(
					'date'          => esc_html__( 'Mais recentes', 'extra' ),
			...
			'_blog_feed_page_order'               => array(
				'title'       => esc_html__( 'Ordem de classificação', 'extra' ),
				'type'        => 'select',
				'default'     => 'desc',
				'options'     => array(
					'desc' => esc_html__( 'Descendente', 'extra' ),
					'asc'  => esc_html__( 'Ascendente', 'extra' ),
				),
				'description' => esc_html__( 'Escolha uma ordem de classificação.', 'extra' ),
			),
			'_blog_feed_page_show_author'         => array(
				'title'       => esc_html__( 'Exibir autores', 'extra' ),
				'type'        => 'select',
				'options'     => array(
					'1' => esc_html__( 'Exibir autor', 'extra' ),
					'0' => esc_html__( "Não exibir autor", 'extra' ),
				),
				'description' => esc_html__( "Ativar ou desativar a exibição do autor de cada postagem.", 'extra' ),
			),
			'_blog_feed_page_show_categories'     => array(
				'title'       => esc_html__( 'Exibir categorias', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => esc_html__( 'Exibir categorias', 'extra' ),
					'0' => esc_html__( "Nãoexibir categorias", 'extra' ),
				),
				'description' => esc_html__( "Ativar ou desativar a exibição das categorias de cada postagem.", 'extra' ),
			),
			'_blog_feed_page_show_featured_image' => array(
				'title'       => esc_html__( 'Mostrar Imagem em Destaque', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => esc_html__( 'Mostrar Imagem em Destaque', 'extra' ),
					'0' => esc_html__( "Não mostrar a imagem em destaque", 'extra' ),
				),
				'description' => esc_html__( "Ativar ou desativar a exibição da imagem em destaque de cada postagem.", 'extra' ),
			),
			'_blog_feed_page_show_ratings'        => array(
				'title'       => esc_html__( 'Mostrar classificações', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => esc_html__( 'Exibir classificações', 'extra' ),
					'0' => esc_html__( "Não exibir classificações", 'extra' ),
				),
				'description' => esc_html__( "Ativar ou desativar a exibição das estrelas de classificação de cada postagem.", 'extra' ),
			),
			'_blog_feed_page_content_length'      => array(
				'title'       => esc_html__( 'Conteúdo', 'extra' ),
				'type'        => 'select',
				'options'     => array(
					'excerpt' => esc_html__( 'Mostrar conteúdo', 'extra' ),
					'full'    => esc_html__( "Mostrar conteúdo completo", 'extra' ),
				),
				'description' => esc_html__( "Exibir o excerto de mensagens ou conteúdo completo. De conteúdo completo, truncará para a tag mais se usada.", 'extra' ),
			),
			'_blog_feed_page_show_date'           => array(
				'title'       => esc_html__( 'Exibir data', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => esc_html__( 'Exibir data', 'extra' ),
					'0' => esc_html__( "Não exibir data", 'extra' ),
				),
				'description' => esc_html__( "Ativar ou desativar a exibição de cada data de postagem.", 'extra' ),
			),
			'_blog_feed_page_date_format'         => array(
				'title'       => esc_html__( 'Formato da data', 'extra' ),
				'type'        => 'input',
				'default'     => 'M j, Y',
				'description' => esc_html__( 'O formato para a exibição da data no formato PHP date ()', 'extra' ),
			),
		);
	}
...
class Extra_Sitemap_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'sitemap-page-template', esc_html__( 'Opções do modelo de página do sitemap', 'extra' ), 'page-template-sitemap.php' );
	}
	...
	function display( $post ) {
		$sections = array(
			...
			'recent_posts' => esc_html__( 'Artigos recentes', 'extra' ),
	...
	function fields() {
		...
		$this->fields = array(
			'_extra_sidebar_location' => array(
				'title'      => esc_html__( 'Localização do sidebar', 'extra' ),
				'type'       => 'select',
				'options'    => array(
					''      => sprintf( esc_html__( 'Padrão (%s)', 'extra' ), ucwords( $global_sidebar_location ) ),
					'right' => esc_html__( 'Direita', 'extra' ),
					'left'  => esc_html__( 'Esquerda', 'extra' ),
					'none'  => esc_html__( 'Nenhum sidebar', 'extra' ),
				),
				'attributes' => array(
		...
		$this->fields['_extra_sidebar'] = array(
			'title'   => esc_html__( 'Escolha Sidebar/Widget Area', 'extra' ),
			'type'    => 'select',
			'options' => array(),
		);
		...
		if ( !empty( $wp_registered_sidebars[ $global_sidebar ] ) ) {
			...
			$this->fields['_extra_sidebar']['options'][''] = sprintf( esc_html__( 'Padrão (%s)', 'extra' ), $sidebar['name'] );
		} else {
			$this->fields['_extra_sidebar']['options'][''] = esc_html__( 'Padrão', 'extra' );
		}
		...
		if ( 'post' == $post->post_type ) {
			$this->fields['_extra_featured_post'] = array(
				'title' => esc_html__( 'Artigos em destaque', 'extra' ),
			);
			...
			if ( 'on' == et_get_option( 'extra_show_author_box', 'on' ) ) {
				$this->fields['_extra_hide_author_box'] = array(
					'title' => esc_html__( 'Esconder box do autor', 'extra' ),
				);
			} else {
				$this->fields['_extra_show_author_box'] = array(
					'title' => esc_html__( 'Exibir box do autor', 'extra' ),
				);
			}
			...
			if ( 'on' == et_get_option( 'extra_show_related_posts', 'on' ) ) {
				$this->fields['_extra_hide_related_posts'] = array(
					'title' => esc_html__( 'Esconder posts relacionados', 'extra' ),
				);
			} else {
				$this->fields['_extra_show_related_posts'] = array(
					'title' => esc_html__( 'Exibir posts relacionados', 'extra' ),
				);
			}
		...
		if ( extra_check_feature_availability_in_post_type( $post->post_type, 'hide_title_meta_in_single' ) ) {
			$this->fields['_post_extra_title_meta_hide_single'] = array(
				'title'                   => esc_html__( 'Esconder título &amp; Meta no Post', 'extra' ),
		...
		if ( extra_check_feature_availability_in_post_type( $post->post_type, 'hide_featured_image_in_single' ) ) {
			$this->fields['_post_extra_featured_image_hide_single'] = array(
				'title'                   => esc_html__( 'Esconder imagens recentes no Post', 'extra' ),
				'value_sanitize_function' => 'wp_validate_boolean',
				'type'                    => 'checkbox',
			);
		}
		...
		if ( et_get_post_meta_setting( 'all', 'rating_stars' ) && in_array( $post->post_type, extra_get_rating_post_types() ) ) {
			$this->fields['_post_extra_rating_hide'] = array(
				'title'                   => esc_html__( 'Enconder postagens mais visitadas', 'extra' ),
				'value_sanitize_function' => 'wp_validate_boolean',
				'type'                    => 'checkbox',
			);
		}
	...
	if ( in_array( $hook, array( 'post.php', 'post-new.php' ) ) ) {
		...
		if ( 'post' == $typenow || EXTRA_PROJECT_POST_TYPE == $typenow ) {
			...
			wp_localize_script( 'extra_admin_posts_js', 'EXTRA', array(
				'label_breakdown_title'  => esc_html__( 'Título:', 'extra' ),
				'label_breakdown_rating' => esc_html__( 'Classificação (%):', 'extra' ),
				'label_breakdown_number' => esc_html__( 'Repartição #', 'extra' ),
			) );
		}
