...
function extra_get_the_post_score_bar( $args = array() ) {
	...
	$bar = sprintf('<span class="score-bar" style="width:%1$d%%;background-color:%2$s;"><span class="score-text">%3$s</span></span>',
		...
		esc_attr( $color ),
		sprintf( esc_html__( 'Pontuação %1$d%%', 'extra' ), intval( $breakdown_score ) )
	);
	return $bar;
}
...
function extra_the_post_rating_stars_with_rating_count( $post_id = 0 ) {
	...
	printf(
		...
		esc_html( sprintf( _n( '%d voto', '%d votos', $rating_count ), $rating_count ) )
	);
}
		...
		case 'authors':
			...
			foreach ( $authors as $author ) {
				$items .= sprintf(
					...
					esc_attr( sprintf( __( 'Postado por: %s', 'extra' ), $author->display_name ) ),
					esc_html( $author->display_name )
				);
			}
			break;
